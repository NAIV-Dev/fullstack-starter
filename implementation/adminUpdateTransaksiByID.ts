...
import { T_adminUpdateTransaksiByID } from "../types/api/adminUpdateTransaksiByID";
...
export const adminUpdateTransaksiByID: T_adminUpdateTransaksiByID = async req => {
  ...
  trx.sudah_diambil = req.body.sudah_diambil ?? trx.sudah_lunas ?? false;
  trx.sudah_lunas = req.body.sudah_lunas ?? trx.sudah_lunas ?? false;
  trx.total_harga = req.body.total_harga ?? trx.total_harga;
  trx.catatan = req.body.catatan ?? trx.catatan ?? '';
